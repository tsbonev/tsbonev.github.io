(function(){
	...
	function defaultState(){
		return {
			...
			guests: [],
			tables: [],
			ui: { selectedTableId: null, zoom: 1, guestSort: 'unassignedFirst', guestUnassignedOnly: false, snap: false, grid: 64, showGrid: true, language: 'bg' },
			_history: { past: [], future: [], suppress: false }
		};
	}

	function migrate(obj){
		if(!obj.ui) obj.ui = { selectedTableId: null, zoom: 1 };
	...
	function removeTable(id){
		...
		if(state.ui.selectedTableId === id) state.ui.selectedTableId = null;
		save();
		window.TablePlanner.render();
	}
