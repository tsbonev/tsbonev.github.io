(function(){
	...
	function render(){
		...
		// Use base grid size
		const baseGridSize = state.ui.grid || 12;
		canvas.style.setProperty('--grid', baseGridSize + 'px');
		canvas.style.backgroundImage = state.ui.showGrid === false ? 'none' : '';

		for(const table of state.tables){
			if(table.type === 'rect'){
				renderRect(canvas, table, state.ui.selectedTableId===table.id);
			}else if(table.type === 'separator'){
				renderSeparator(canvas, table, state.ui.selectedTableId===table.id);
			}else{
				renderCircle(canvas, table, state.ui.selectedTableId===table.id);
			}
		}

		const seatInput = document.getElementById('seatCountInput');
		const sel = state.tables.find(t=>t.id===state.ui.selectedTableId);
	...
	function applySeatVisual(seatEl, table, index){
		...
		if(g){
			...
			seatEl.title = g.name + ' (Seat ' + (index+1) + ')';
	...
	function renderGuestSidebar(){
		...
		const guestIdToAssignment = buildGuestAssignmentIndex(state);

		let guests = state.guests.slice();
		if(state.ui.guestSort === 'name'){
			guests.sort((a,b)=>a.name.localeCompare(b.name));
		}else if(state.ui.guestSort === 'assignedFirst'){
			guests.sort((a,b)=>{
		...
		if(state.ui.guestUnassignedOnly){
		...
		for(const g of guests){
			...
			status.textContent = assign ? `Table ${assign.tableLabel} • Seat ${assign.seat + 1}` : 'Unassigned';
		...
		if(sortSelect) sortSelect.value = state.ui.guestSort;
		const unassignedOnly = document.getElementById('guestUnassignedOnly');
		if(unassignedOnly) unassignedOnly.checked = !!state.ui.guestUnassignedOnly;
	}

	function buildGuestAssignmentIndex(state){
		const map = new Map();
